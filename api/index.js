...
module.exports = {
    ...
    get: (req, res, next) => {
        if (req.body.githubRedir === "true")
            res.status(200).sendFile(path.resolve(`${__dirname}/../static/notapi.html`), (err) => {
                if (err) next(err);
                else next();
            });
        else {
...
function sendError(res, text) {
    res.status(404).send(fs.readFileSync(path.resolve(`${__dirname}/../static/error.html`), "utf-8").replaceAll("{{variable}}", text));
}